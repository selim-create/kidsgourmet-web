...
export default function SponsoredPostCard({ post, categories, variant = 'default' }: SponsoredPostCardProps) {
  ...
  if (variant === 'hero') {
    const LinkWrapper = hasGamTracking ? 'a' : Link;
    const linkProps = hasGamTracking 
      ? { 
          href: finalUrl,
          ...(isExternalLink && { 
            target: '_blank', 
    ...
    return (
      <div className="relative">
        ...
        <LinkWrapper {...linkProps} className="group relative block rounded-[2.5rem] overflow-hidden shadow-xl aspect-[16/9] md:aspect-[21/9]">
  ...
  return (
    <article className="flex flex-col group h-full relative">
      ...
      {/* @ts-ignore - Dynamic component type */}
      <LinkWrapper {...linkProps} className="block overflow-hidden rounded-[2rem] mb-4 relative aspect-[4/3]">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <h3 className="font-display font-bold text-xl text-slate-800 mb-3 leading-snug group-hover:text-orange-500 transition-colors font-sans">
          {/* @ts-ignore - Dynamic component type */}
          <LinkWrapper {...linkProps} dangerouslySetInnerHTML={{ __html: post.title.rendered }} />
        </h3>
        ...
        <div className="flex items-center gap-2 pt-4 border-t border-gray-100">
          {/* @ts-ignore - Dynamic component type */}
          <LinkWrapper {...linkProps} className="text-xs font-bold text-slate-700 hover:text-orange-500 transition-colors">

...
export const RELATED_NEWSLETTER_SLUGS = [
  'aksam-ne-pisirsem',
  'iyi-yasam-notlari',
] as const;

export const ALLOWED_NEWSLETTER_SLUGS = [
  ...KIDSGOURMET_NEWSLETTER_SLUGS,
  ...RELATED_NEWSLETTER_SLUGS,
] as const;
...
export type AllowedNewsletterSlug = (typeof ALLOWED_NEWSLETTER_SLUGS)[number];

export interface HipostaNewsletterOption {
  slug: AllowedNewsletterSlug;
  name: string;
...
export const DEFAULT_NEWSLETTER_BY_SOURCE: Record<NewsletterSourceId, AllowedNewsletterSlug> = {
...
export function isAllowedNewsletterSlug(value: string): value is AllowedNewsletterSlug {
  return (ALLOWED_NEWSLETTER_SLUGS as readonly string[]).includes(value);
}

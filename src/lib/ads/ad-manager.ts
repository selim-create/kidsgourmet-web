...
class AdManager {
  ...
  defineInterstitialSlot(slotConfig: AdSlot): void {
    ...
    window.googletag.cmd.push(() => {
      ...
      // IMPORTANT: Out-of-page slots require an explicit display() call
      // The slot element ID is auto-generated by GPT for out-of-page slots
      const slotElementId = slot.getSlotElementId();
      if (slotElementId) {
        window.googletag.display(slotElementId);
      } else {
        console.warn(`Interstitial slot element ID not available: ${adUnitPath}`);
      }
    });
  }
